...
describe('Add Product On Order (E2E)', async () => {
  ...
  test('[POST] /orders/:orderId/productsVariants [201]', async () => {
    const response1 = await request(app.getHttpServer())
      .post(`/orders/${order.id.toString()}/productsVariants`)
      .set({
    ...
    const orderProductVariantOnDatabase =
      await prisma.orderProductsVariants.findFirst({
        where: {
    ...
    expect(orderProductVariantOnDatabase).toBeTruthy()
    expect(orderProductVariantOnDatabase?.quantity).toEqual(10)

    const productVariantInventoryOnDatabase =
      await prisma.productVariantInventory.findFirst({
        where: {
          id: productVariantInventory.id.toString(),
        },
      })

    expect(productVariantInventoryOnDatabase?.quantity).toEqual(0)
  })
})
